...
**Séances 1 à 8 closes côté dossier et côté outil.** Le bureau du RSSI est **complet à 16 pages sur 16** pour les huit séances tenues (F7, F12, F14, F19 clos) — reste une tenue en longueur à resserrer, point ouvert sans enjeu de points manquants. La séance 7 est **entièrement faite** : les deux TD, les deux TP, le livrable `D7` (5 pts) et la sous-section 7 de la note — coûts du plan de traitement lus dans `translog-b` (82K €/an, 13/13 mesures), les deux acceptations formelles créées comme objets `Risk acceptances`. **La séance 8 est entièrement faite** : le TD 1 (note collective, deux pages individuelles), les deux TP saisis dans `translog-b` (30 exigences de clauses évaluées, 15 contrôles d'annexe A investigués, 16,1 % de couverture), le livrable `D8` (2 pts) et la sous-section 8 de la note. Reste le CM (non noté). Tout ce qui relève de la séance 9 n'est pas « en retard » — elle n'a pas eu lieu.
...
| **D8** | `…/Session-8/2-Labs/D8-declaration-d-applicabilite-et-perimetre-smsi.md` | **Les 4 sections exigées** : périmètre du SMSI en quatre paragraphes (activités, entités, interfaces, exclusions assumées), déclaration d'applicabilité (**16,1 % de couverture** affichée en tête, 15 contrôles investigués, cohérence croisée avec D4/D7 vérifiée), registre des exclusions (`A.8.28`), synthèse pour la direction en dix lignes |
...
### 🔴 Avant la remise du 17 septembre — ce qui dépend encore de nous

**Plus aucun livrable ni aucune page individuelle en souffrance sur les séances 1 à 8.** Les deux chantiers qui occupaient cette section (les deux pages S7, la page S5 de Miguel) sont clos par **F19** — bureau du RSSI à **16 pages sur 16**. Ce qui reste avant la remise est de la **qualité**, pas des points manquants :

| # | Objectif | Où | Enjeu | Charge |
|---|---|---|---|---|
| **1** | **F17 · trancher cinq écarts de fond mis en attente** — trois mesures de D4/D7 datées deux fois sur le même contrôle (`M1`/`PT-03`, `M2`/`PT-06`, `M4`/`PT-02`), le titre/genre du dirigeant de la filiale, les porteurs nominatifs de D4 à reprendre en rôles, la table de correspondance des niveaux de risque dans la note, les jalons de D6 sans date | D4, D6, D7, note de stratégie | exactitude et cohérence, aucun point direct en jeu mais lu en soutenance | Moyenne |
| **2** | **F19 · resserrer les seize pages du bureau du RSSI à une page franche** — toutes dépassent aujourd'hui (574 à 864 mots) | `Session-n/1-CISO-desk/` | **posture de RSSI — 3 pts sur 10, coef. 1** | Moyenne |
...
│   ├── fixes.md                       ← journal détaillé des corrections (F1→F19)
