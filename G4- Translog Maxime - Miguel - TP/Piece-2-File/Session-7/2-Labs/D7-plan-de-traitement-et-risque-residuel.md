...
| **Réduire** — ER2 : données de préparation altérées (`Very likely × Important` = `High`) | **`PT-09`** Rotation du secret du compte de service `LOG-SA-04` : un secret par site, en coffre | DSI de la filiale · **14/01/2027** | Build 5 000 € + 5 jp · run 500 € + 2 jp/an · amort. 3 ans · **4 000 €/an** | Ferme l'accès par secret partagé de tout initié — `Very likely → Likely` |
