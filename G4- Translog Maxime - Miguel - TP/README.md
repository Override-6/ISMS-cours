...
- `2-Labs/D8-declaration-d-applicabilite-et-perimetre-smsi.md` — **le livrable D8 (2 points), TP 2**. Périmètre du SMSI en quatre paragraphes (activités, entités et systèmes, interfaces, exclusions assumées — dont l'automatisation d'E4) ; déclaration d'applicabilité, **16,1 % de couverture** affichée en tête, cohérence croisée vérifiée avec D4 (aucun orphelin sur les écarts majeurs `C3`/`C4`) et D7 (trois mesures — `PT-01`, `PT-04`, `PT-12` — et trois contrôles inclus — `A.5.24`, `A.6.3`, `A.7.4` — encore sans contrepartie, signalés) ; registre des exclusions repris tel que saisi ; synthèse pour la direction en dix lignes. **Sous-section 8 de la note de stratégie rédigée** dans le même mouvement.
