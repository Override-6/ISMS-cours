...
| Marquage du Top 5 dans la liste des actifs | Miguel Monereo |
...
|---|---|---|---|
| 1a | Domaine de la filiale sous revue | Oui | `MERIDIAN-LOGISTIQUE`, *parent domain* `Global` |
| 1b | Second domaine (autre bout de la dépendance §6) | Oui — **présent**, contrairement à l'hypothèse de départ | `MERIDIAN-SANTE`, parent `Global`, description « filiale santé, quatre établissements de soins » |
| 1c | Périmètre associé à la filiale | Oui | `MERIDIAN-LOGISTIQUE-FINAL`, domaine `MERIDIAN-LOGISTIQUE` |
| 2 | Actifs de la séance 2 | Oui **17** | 4 primaires `LOG-PA-01` à `04` + 13 supports `LOG-SA-01` à `13` (noms exacts au §1.1) |
| 3a | Référentiel importé en séance 3 | Oui | `International standard ISO/IEC 27001:2022`, provider ISO/IEC, domaine `Global` |
| 3b | Évaluation de conformité remplie en séance 4 | Oui | `MERIDIAN - ISO/IEC 27001:2022 - initial assessment`, progression 9 %, périmètre `MERIDIAN-LOGISTIQUE/MERIDIAN-LOGISTIQUE-FINAL` |
| 4 | Module EBIOS RM, page `EBIOS RM studies` | Oui **liste vide** avant la séance | — |
...
|---|---|---|---|---|
| **ER1** | `LOG-PA-01` Exécution des flux logistiques | `Availability` | **Critical** | Oui |
| **ER2** | `LOG-PA-01` Exécution des flux logistiques | `Integrity` | **Important** | Oui |
| **ER3** | `LOG-PA-02` Maintien de la chaine du froid | `Availability` + `Integrity` | **Critical** | Oui |
| **ER4** | `LOG-PA-04` Conformite contractuelle | **`Proof`** | **Important** | Oui |
| **ER5** | `LOG-PA-01` Exécution des flux logistiques | `Availability` | **Critical** | Oui |
| **ER6** | `LOG-PA-02` Maintien de la chaine du froid | `Confidentiality` | **Significant** | Oui |
...
- Le champ `Qualifications` propose bien plus que les quatre valeurs attendues : `Authenticity`, `Availability`, `Confidentiality`, `Environmental`, `Financial`, `Governance`, `Human`, `Image`, `Integrity`, `Proof`… Les quatre du guide suffisent au TD, mais l'inventaire réel est à connaître.
...
|---|---|---|---|---|---|---|---|---|
| **1** | Cybercriminel | `Organized crime` | `Strong` | `Important` | `Important` | Oui | ER1, ER2 | **Highly relevant** |
| **2** | Attaquant passant par l'intégrateur | `Other` | `Significant` | `Important` | `Moderate` | Non | ER1 | **Fairly relevant** |
| **3** | Initié de l'Exploitation | `Avenger` | `Significant` | `Significant` | `Low` | Oui | ER2 | **Fairly relevant** |
| **4** | Concurrent | `Competitor` | `Low` | `Significant` | `Low` | Oui | ER4, ER6 | **Partially relevant** |
| **5** | Militant / hacktiviste | `Activist` | `Low` | `Limited` | `Low` | Non | ER3 | **Irrelevant** |
...
## 7. Marquage du Top 5 dans l'instance
...
> **Contrainte de l'outil relevée** : une étiquette doit être **alphanumérique et faire au plus 36 caractères**. Le premier essai, `Top 5 D2`, a été **refusé** (l'espace n'est pas accepté) ; l'étiquette retenue est **`Top5`**, sans espace. À citer dans D2 pour que l'étiquette du dossier et celle de l'instance coïncident.
>
...
**Fait** : D2 affirmait « instance *et* dossier », c'est désormais vrai. Liste d'actifs recapturée dans `Session-2/3-Evidence/` (`S2-05bis-assets-list-Top5-page1..3-of-3.jpg`) et dans `Session-5/3-Evidence/` (`S5-05-top5-marque-assets-page1..3.jpg`).
...
## 9. Relecture de clôture

- Table d'inventaire produite, **nom exact observé** pour chaque objet ; l'écart (`MERIDIAN-SANTE` présent, `API` vs « interface ») est **signalé, rien n'a été recréé ni renommé**
...
- Les actifs sont **reliés**, pas recréés — **aucun doublon** dans `Assets` (17 avant, 17 après)
...
- Écart de `Pertinence` relevé (couples 4 et 2), **sélection inchangée**
- Ateliers 3, 4 et 5 **vides**, et la raison écrite
- Chaque objet créé porte un **auteur nommé** (compte nominatif, jamais `admin@lockbay.eu`)
