...
| Tooling and implementation effort | 10 | **2** — imports natively into the group tool, but the management system and audit are a real charge | **3** — free, 20 objectives, and a **native library in the instance** (verified on screen): the lightest implementation charge of the three | **3** — free, two levels, its own tracking tool |
| **Weighted total (out of 300)** | **100** | **230** | **205** | **135** |

*Detail — ISO: 30+75+60+45+20 · ReCyF: 90+50+20+15+**30** · Hygiene: 30+25+20+30+30.*

ISO leads ReCyF by 25 points out of 300 and the hygiene guide by nearly 100. **The hygiene guide wins the effort column and loses every other: it is a starting point, not a group backbone.**
...
| **Tooling** | Its own tracking tool, free — but standalone, outside the group instance (grid) | **Native library, already in the instance** — provider ANSSI, v2.5 of 17/03/2026, published 09/07/2026 (import). Nothing to build; the charge is the 20 objectives themselves, the lightest of the three | **Zero marginal cost** — imports natively, and the import is already done: 123 requirements created (import) |
...
|---|---|---|---|
| Weight transfer | Binding nature 30 → **39**, maturity 15 → **6** | ISO **212**, ReCyF **223** → flip | Requires moving 9 points out of 100 — nearly a third of the weight of the already-heaviest criterion — *and* reducing text stability to almost nothing. That is not a fine adjustment, it is a change of doctrine |
| Criterion removal | Drop "evidence enforceable towards third parties" (20) | ISO **170**, ReCyF **185** → flip | This test names what actually protects the recommendation: the client local authorities and the pharmaceutical questionnaire. If the Committee judges enforceable evidence accessory, it should choose differently — and say so explicitly |

**What this sensitivity teaches is that the arithmetic is not what carries the recommendation — the veto rule is.** Both tests flip, and by wider margins than the base gap of 25 points suggests. If the choice rested on the weighted total alone it would be fragile. It rests instead on a **condition** — covering a subsidiary outside NIS 2 scope — that ReCyF fails whatever its score. The recommendation is therefore **robust without being unassailable**, and it rests on two contextual judgements the Committee must validate openly: that evidence towards third parties counts, and that the stability of the text counts.
...
Two provenance notes, because these objects are what the group will be measured on rather than this prose. First, the instance dates and attributes every object it holds: that is what makes an export evidence rather than an assertion. Second, **what is in the library is a matter of fact, not of assumption.** ReCyF *is* present — provider ANSSI, v2.5 of 17/03/2026, published 09/07/2026 — and we found it and chose not to import it: one imports the backbone, not everything available, or the instance stops meaning anything. An earlier draft of this note asserted the opposite, and our own screenshot corrected it.
